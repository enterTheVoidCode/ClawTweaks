...
| 10 | Fan custom curve | 🟡 partial | P5's write+read-back core answered by live widget use 2026-07-05 (port log evening entry): EC round-trips our tables on bytes 1–7; byte[0] anomaly (reads 58/150, never what was written) unresolved; **EC currently holds an A2VM table, not the EX baseline `58,70,74,76,78,80,84,94` — restore it (= P5 no-op write) before the stress row**; fan-control bit stayed OFF throughout |
