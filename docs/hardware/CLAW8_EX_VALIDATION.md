...
| 3 | Controller emu in a game | ⬜ pending human | ~~Blocker: ViGEm and HidHide are NOT installed~~ **Blocker cleared 2026-07-05**: HidHide + usbip-win2 installed and running (verified via PnP/services). Note the default emulation backend is now **VIIPER (usbip-win2)**, not ViGEm — ViGEm is legacy, only installed via explicit `-Only vigem` (`Setup-Tools.ps1:211`). Judge this row against VIIPER |
